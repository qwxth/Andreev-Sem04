...
async def lifespan(app: FastAPI):
    logger.info("=== RAG-сервис запущен ===")
    yield
    logger.info("=== RAG-сервис остановлен ===")


app = FastAPI(title="RAG Service", version="2.1", lifespan=lifespan)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
async def remove_document(document_id: str):
    try:
        ...
        if deleted == 0:
            raise HTTPException(404, f"Документ '{document_id}' не найден")
        return {
            "success": True,
            "deleted": deleted,
            "message": f"Документ {document_id} удалён ({deleted} записей)"
        }
...
async def search(req: SearchRequest):
    ...
    try:
        ...
        if not docs:
            create_session(sid, [], req.question)
            return {
        ...
        # top_m документов идут в контекст генерации
        ...
        logger.info(f"Сессия {sid} создана, документы: {doc_ids}")

        return {
            "question": req.question,
            "answer": answer,
            "relevant_documents": [d.model_dump() for d in docs],  # все top_k
            "documents_found": len(docs),
            "documents_used_count": len(context_docs),             # top_m
            "session_id": sid,
            "document_ids": doc_ids   # ID документов в контексте (top_m)
        }
    except Exception as e:
